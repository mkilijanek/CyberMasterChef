...
export const argon2i: Operation = {
  id: "crypto.argon2i",
  name: "Argon2i",
  description: "Derives an Argon2i hash string or hex digest.",
  input: ["bytes", "string"],
  output: "string",
  args: [
  ...
  run: async ({ input, args }) => {
    ...
    const { argon2i } = (await import("hash-wasm")) as {
      argon2i: (options: ReturnType<typeof parseArgon2Options>) => Promise<string>;
    };
    const options = parseArgon2Options(input, args);
    const hash = await argon2i(options);
