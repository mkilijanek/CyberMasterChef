...
export const bitShiftRight: Operation = {
  id: "bytes.bitShiftRight",
  name: "Bit Shift Right",
  description: "Shifts each byte right by the selected amount.",
  input: ["bytes", "string"],
  output: "bytes",
  ...
  run: ({ input, args }) => {
    ...
    return {
      type: "bytes",
      value: Uint8Array.from(bytes, (byte) => byte >>> amount)
    };
  }
};
