...
export const avroToJson: Operation = {
  id: "format.avroToJson",
  name: "Avro To JSON",
  ...
  run: async ({ input, args }) => {
    ...
    const schema = parseJson("format.avroToJson", schemaRaw);
    const { Type } = (await import("avsc")) as {
      Type: { forSchema: (schema: unknown) => { fromBuffer: (value: Buffer) => unknown } };
    };
