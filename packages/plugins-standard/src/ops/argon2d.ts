...
export const argon2d: Operation = {
  id: "crypto.argon2d",
  name: "Argon2d",
  description: "Derives an Argon2d hash string or hex digest.",
  input: ["bytes", "string"],
  output: "string",
  args: [
  ...
  run: async ({ input, args }) => {
    ...
    const { argon2d } = (await import("hash-wasm")) as {
      argon2d: (options: ReturnType<typeof parseArgon2Options>) => Promise<string>;
    };
    const options = parseArgon2Options(input, args);
    const hash = await argon2d(options);
